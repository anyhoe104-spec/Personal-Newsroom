...
(async () => {
  ...
  try {
    ...
    await page.locator('.card:has(.like.selected) .bad').click();
    assert.deepEqual(await page.locator('.card').evaluateAll(nodes => nodes.map(n => n.dataset.id)), beforeVote);
    ...
    const stable = await page.locator('.card').evaluateAll(nodes => nodes.map(n => n.dataset.id));
    await page.locator('.save.selected').first().click();
    assert.deepEqual(await page.locator('.card').evaluateAll(nodes => nodes.map(n => n.dataset.id)), stable);
    await page.locator('.save:not(:disabled)').first().click();
    ...
    const download = page.waitForEvent('download'); await page.locator('#downloadBackup').click();
    const downloaded = await download; assert.equal(downloaded.suggestedFilename(), 'newsroom-backup.json');
    const downloadedPath = await downloaded.path();
    assert.deepEqual(JSON.parse(fs.readFileSync(downloadedPath, 'utf8')), JSON.parse(backup));
    assert.equal(await page.locator('#importBackup').getAttribute('accept'), null);
    await page.locator('#importBackup').setInputFiles({ name: 'bad.json', mimeType: 'application/json', buffer: Buffer.from('{bad') });
    await page.waitForFunction(() => document.getElementById('settingsStatus').textContent.includes('取り込めません'));
    ...
    await page.locator('#importBackup').setInputFiles(downloadedPath);
    await page.waitForFunction(() => document.getElementById('settingsStatus').textContent.includes('統合'));
    assert.match(await page.locator('#settingsStatus').innerText(), /統合/);
    await page.locator('#backupText').evaluate(n => { n.closest('details').open = true; });
    await page.locator('#backupText').fill('\uFEFF' + backup); await page.locator('#importPastedBackup').click();
    assert.equal(await page.locator('#backupText').inputValue(), '');
    ...
    // View-local filters and a small, chronological first page for large histories.
    await page.locator('#tabs button').last().click();
    await page.locator('[data-view="insights"]').click();
    assert.equal(await page.locator('.insight').count(), 4);
    assert.equal(await page.locator('.hero').isVisible(), false);
    await page.locator('[data-view="today"]').click();
    assert.equal(await page.locator('#tabs .active').innerText(), '卵・食品開発');
    assert.equal(await page.locator('.hero').isVisible(), true);
    await page.evaluate(() => {
      const state = NewsroomLearning.empty();
      state.feedback.egg = Array.from({ length: 1000 }, (_, i) => ({ id: 'history-' + i, category: 'egg', title: '卵 &amp;middot; 食品', source: 'QA', url: 'https://example.org/' + i, value: 'like', at: new Date(Date.now() - i * 1000).toISOString(), summary: ['&lt;img src=x onerror=alert(1)&gt;'] }));
      localStorage.setItem(NewsroomLearning.KEY, JSON.stringify(state));
    });
    await page.reload();
    const historyStarted = Date.now();
    await page.locator('[data-view="history"]').click();
    assert.equal(await page.locator('.card').count(), 30);
    assert.equal(await page.locator('.card').first().getAttribute('data-id'), 'history-0');
    assert.equal(await page.locator('.card .title').first().innerText(), '卵 · 食品');
    assert.equal(await page.locator('.card img').count(), 0);
    assert.match(await page.locator('.card .summary').first().innerText(), /<img/);
    console.log(`History first page (1000 stored votes): ${Date.now() - historyStarted} ms`);
    await page.locator('.more-history').click(); assert.equal(await page.locator('.card').count(), 60);
    await page.evaluate(() => window.scrollTo(0, 700));
    assert.ok(Math.abs(await page.locator('.category-nav').evaluate(n => n.getBoundingClientRect().top)) < 2);
    await page.locator('[data-view="today"]').click();
    assert.equal(await page.locator('.like.selected').count(), 0);
